...
Каталог уезжает клиентам не из этого репозитория, а релизом сайта: workflow **release**
(`dev` или `prod`) собирает `kalitka-marketplace-<версия>.zip` из `.agents` и `plugins`,
кладёт его в GitHub Release и в бакет стенда и регистрирует черновик на сайте. Публикацию
нажимают в админке. Приложение Kalitka раскладывает архив в `~/.codex/kalitka-marketplace`
и регистрирует его как локальный маркетплейс `kalitka`.

Плагин обновляется на машинах только через новую `version` в его `plugin.json`: Codex
копирует плагин в кеш при установке и на изменения каталога не смотрит.
