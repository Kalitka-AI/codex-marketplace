...
- `plugins/<name>/` — плагин: манифест `.codex-plugin/plugin.json`,
  MCP-серверы в `.mcp.json`, README с деталями подключения.

Чтобы добавить плагин: создайте папку в `plugins/`, опишите манифест и
`.mcp.json`, добавьте запись в `marketplace.json` и строку в таблицу выше.
